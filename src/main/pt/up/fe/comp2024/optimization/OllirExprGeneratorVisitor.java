    ...
    private OllirExprResult visitMethodInvocation(JmmNode node, Void unused) {
        ...
        if(typeFunction.equals("invokevirtual")) {
            ...
            if(table.getMethods().contains(node.get("value")))
            {
                ...
                for(int i = 0; i < paramsAux.size(); i++) {
                    if (paramsAux.get(i).getType().isArray()) {
                        if (!node.getJmmChild(i+1).getKind().equals(ARRAY_CREATION_EXPR.toString()) && !node.getJmmChild(i+1).getKind().equals(NEW_ARRAY_EXPR.toString()) && !node.getJmmChild(i+1).getKind().equals(VAR_REF.toString()) && !node.getJmmChild(i+1).getKind().equals(METHOD_CALL_EXPR.toString())) {
            ...
            else
            {
                if(table.getMethods().contains(node.get("value")))
                {
                    var auxiliar = table.getReturnType(node.get("value"));
                    if(auxiliar.isArray())
                    {
                        var temp = OptUtils.getTemp();
                        ollirType = ".array"+ OptUtils.toOllirType(TypeUtils.getExprType(node, table));
                        code = temp  + ollirType;
                        computation.append(temp).append(".array").append(ollirType).append(SPACE).append(ASSIGN).append(".array").append(ollirType).append(SPACE);
                    }
                    else {
                        var temp = OptUtils.getTemp();
