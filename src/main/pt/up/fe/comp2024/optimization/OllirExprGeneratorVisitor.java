            ...
            if(table.getMethods().contains(node.get("value")))
            {
                ...
                for(int i = 0; i < paramsAux.size(); i++) {
                    if (paramsAux.get(i).getType().isArray()) {
                        if (!node.getJmmChild(i+1).getKind().equals(ARRAY_CREATION_EXPR.toString())) {
                            ...
                            for (int j = 0; j < node.getNumChildren() - 1 - i; j++) {
                                computation.append(temp2).append("[").append(j).append(".i32").append("]").append(ollirType).append(ASSIGN).append(ollirType).append(SPACE).append(visit(node.getJmmChild(j + 1 + i)).getCode()).append(END_STMT);
                            }
                        }
                        /*
                        else if(node.getJmmChild(i+1).getKind().equals(ARRAY_CREATION_EXPR.toString()))
                        {
                            var temp2 = OptUtils.getTemp();
                            ollirType = OptUtils.toOllirType(paramsAux.get(i).getType());
                            arraysCode.append(temp2).append(".array").append(ollirType).append(",");
                            computation.append(temp2).append(".array").append(ollirType).append(ASSIGN).append(".array").append(ollirType).append(SPACE).append("new(array, ").append(node.getJmmChild(i + 1).getNumChildren()).append(".i32").append(" )").append(".array").append(ollirType).append(END_STMT);
                            for(int j = 0; j < node.getJmmChild(i+1).getNumChildren(); j++)
                            {
                                computation.append(temp2).append("[").append(j).append(".i32").append("]").append(ollirType).append(ASSIGN).append(ollirType).append(SPACE).append(visit(node.getJmmChild(i+1).getJmmChild(j)).getCode()).append(END_STMT);
                            }
                        }
